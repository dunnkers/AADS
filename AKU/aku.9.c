...
void incrSum(int *depth, int height, long long int l, long long int r, 
  unsigned long long int *sum, long long int value, int *numberOfPasses) {
    ...
    long long int dude1 = l - leftBound; // distance away from `l`. positive # is left from bound
    dude1 = dude1 > 0 ? (long long int)(ceil((double)dude1 / (double)increment)) : 0;
    dude1 = leftBound + increment * dude1;

    long long int dude2 = rightBound - r; // distance away from `r`. positive # is right from bound
    dude2 = dude2 > 0 ? (long long int)(ceil((double)dude2 / (double)increment)) : 0;
    dude2 = rightBound - increment * dude2;

    long long int amt = dude2 - dude1;
    amt = amt / increment + 1;
    *sum += amt * value;
    *numberOfPasses += amt;
